...
*** TODO Fix the URI scheme in Eview/Eviewtype
...
package, in the map of composition classes (from the Viewpoint), then we add
them to the concreteToVirtualClass map, and iterate on their features.
...
Regardless, getEContainingClass... but that's not even a containing reference.
Weird.
