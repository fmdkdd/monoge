...
* [2018-11-26 lun.]
** Putting the formalism in the journal paper           :emfviews:formalisim:
...
- At first I went with EOL, because that would have been more interesting for
  ...
  option (since with OCL you may need have duplicated computation between
  queries, with no way to reuse them), but we still need to find the adequate
  syntactic option.
...
3. I thought I would be able to resue the OCL query objects, but apparently
...
for isntance has the hardcoded type "Row".  There is no corresponding
...
above, but this is probably something we can achieve before the end of the
project.
