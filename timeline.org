...
the Java API.  It does seem to produce an XMI, but I can't seem to execute the
benchmarks.
...
* [2019-09-03 mar.]
** Trying to bench modeljoin (contd.)                             :modeljoin:
I wanted to add the component part in the Modeljoin query.  Modeljoin supports
multiple joins in the same query, but it doesn't accept joining on target
element created by other joins.  So we can't join Log and ClassDeclaration as
LogClass then join that with uml.Component.

So I guess I have to join them separately.

Hmm, keeping Component.packagedElements is not enough to see anything.  There's
a warning saying that reference has no concrete subclass, and indeed I don't see
any Usage class in there.  Well that's okay.  I'll test the scalability of the
thing instead.

Running the QVT-O transformation with a 100k trace: 108s.  Opening it in MoDisco
takes just a few seconds.

More than 600sec for EMF Views to try to open the view and then I ran out of the
patience.  The O(n^2) ECL matching is killing us.

Trying to find a workaround... but there is no quick one.  Lazily populating the
virtual values is the Right Thing To Do.  But it's also more error-prone, and it
requires to change the interface a bit: we need to know, for each virtual
feature, what is the corresponding query that can be used to populate it.  We
also need to know that a feature will be populated at a later point, for
compatibility with view weaving models which contain explicit links.

That means a third mechanism to populate views: 1) pre-computed weaving
models, 2) matching models, 3) lazy matching models.

And, it's not even the optimal solution which I outlined before, which was to
provide an EOL file that builds the weaving model.  It is strictly better
because with separate lazy matching queries, we may have redundant computation
that we cannot easily detect and share.  With the EOL file, the user is in
control and can avoid the redundant computation.  But it is less user-friendly
than VPDL.

* [2019-09-04 mer.]
** Lazily matching ECL rules                         :ecl:modeljoin:emfviews:
Discussing with H., I'll try to implement the lazy loading of ECL matchers by
default.  Which still requires some work to do correctly, but at least we don't
have to change the interface.

I think it's a good call, since the default matching algorithm is never the
thing you want anyway.

So new plan: when a matching model is provided, do not execute it in
ViewResource to build a weaving model.  Instead, parse it and grab the match
rules out of it and register the corresponding features as lazy (a good place to
do some sanity checks).

Then, when the feature is eGet on virtual objects, check if it's lazy, and ask
ECL to execute only that rule for the current object.  Cache the result and we
are done.

Does ECL have an API for executing only a match rule instead of a whole ECL
module?

Tried on a prototype, and it seems we can ask Epsilon to parse the ECL, get
matching rules objects, and there's an API to execute these on plain objects
coming from models.

One thing I still need to check: can we easily pass a VirtualEObject to
MatchRule.match?  Otherwise we'll need to pass these as Epsilon EMF objects,
which can be clunkier.

It works if we pass DynamicEObject straight from resources.  We still need to
add these resources as InMemoryEmfModels to the ECL module so that the correct
getters and setters can be used to probe the resources.

So it's just a matter of separating the two pieces of that work in EMF Views:
doing the parsing/setting up the module and extracting the rules when loading
the viewresource, and then letting the virtual objects call in to the
virtualizer to do the remainder of the work by actually executing the rules,
and populate the feature from the match results.

* [2019-09-05 jeu.]
** Implementing lazily matched ECL rules                       :ecl:emfviews:
Seems to work.  At least I can open a 100k view without waiting for 10min.  It
takes just a few seconds now.  And opening a virtual feature yields the expected
links without a noticeable delay.

MoDisco is slugglish on the view though, not just for virtual features.  And the
Sample Editor just hangs when unfolding the Trace element...

Well, MoDisco is also sluggish with the Modeljoin output, so it's not EMF Views
then.  And the Sample Editor hangs when /opening/ the MJ output.  We are fine.

I forgot one crucial ingredient though: we can navigate virtual features via
their opposite!  Which should also trigger the lazy rule execution.

And it's tricky to get right.  We only have one rule for each virtual
association in ECL.  When accessing the feature that has the same name as the
rule, it works.  When accessing the opposite feature, it has no corresponding
rule.

We can detect that situation.  Let's say I have component<->javaPackages virtual
associations, and a component ECL rule.  Accessing javaPackages, I can see it
has an opposite, and execute the component ECL rule.  The thing is, I'm doing
that from the VirtualEObject that represent a java Package instance, so it makes
no sense to execute the component rule on it!

But luckily, arguments to ECL rule commute: I can use the /same/ rule and pass
the current object as the right-side parameter instead of the left-side.  I
check if the feature has an opposite, and execute the rule the other way around.

Hmm, okay it works... but I get duplicated values in features.  Looks like I'm
adding, then the addInverse from the opposite kicks off.

Fixed it by using addWithoutInverse.

But there's still one issue: we initialize the virtual feature on A and B side
correctly, but we are only telling A that the initialization has been done, so
accessing B will execute the rule again.

We need to ensure all instances correctly set the initialized bit for values
coming from lazy rules.

* [2019-09-06 ven.]
** Implementing lazily matched ECL rules with opposites        :ecl:emfviews:
... is a nightmare.

I think I've got most of it covered, I wrote tests covering the single-valued
and many-valued features cases, with opposites.  All green.

Then MoDisco barfs at me when unfolding a ClassDecl with a ClassCastException!

Debugging...

AH!  I think I got it.  I missed the case where we have one single-valued
feature opposite a many-valued one (which is the case of the simple view used in
the benchmarks).

/Now/ I got it.  It was even trickier than I expected... dealing with opposites
is very nasty.  I should probably refactor that code into something readable.
It's long overdue.

But at least it works!  And my first test on the 100k view is pretty fast.  No
noticeable delay to browse the lazy features.

* [2019-09-09 lun.]
** Master flaw in lazy ECL rules                               :ecl:emfviews:
So I thought everything was fine.  But no!

When executing a rule that populates a virtual feature with an opposite, we
should /not/ mark the rule as executed on the opposite.  It's incorrect.  See:
...
When browsing from Log A, we execute the rule on l:A with c: all ClassDecl.
That is correct.  Let's say we match on a ClassDecl B.  When browsing from c:B,
we expect to see l:A in there, but not only!  We could match with other
instances of Log, and in fact that's the case in this specific view.

So the fix is to not populate the opposite feature, and just re-execute the
match rule when browsing it.  Premature optimization, root of all evil, I tell
you!

At least I'm happy about getting rid of that spaghetti mess dealing with
opposites.

** Faster ECL matching                                                  :ecl:
Now, the funny thing is that opening a javaClass virtual feature from a Log to a
ClassDecl is instantaneous.  There are only 180 classes to test after all.
Opening the ClassDecl though... takes around 15 seconds to test the 100k Log
instances.

Surely interpreting the whole predicate again for each instance is slow.  To
speed this up further, we could interpret the predicate once into a lambda
(partial eval), and execute it directly on all instances.  Not sure how to turn
the ECL into a lambda though...

Quick check: if I hardcode the rule above as:

#+BEGIN_SRC java
ruleJavaClass(Log l, ClassDeclaration c) {
  return l.getSource().split("\\.")[0].equals(c.getName());
}
#+END_SRC

how much faster is it?

I went with the slightly more generic and roundabout:

#+BEGIN_SRC java
public Match ruleJavaClass(EObject log, EObject classDecl) {
  String source = (String) log.eGet(log.eClass().getEStructuralFeature("source"));
  String name = (String) classDecl.eGet(classDecl.eClass().getEStructuralFeature("name"));

  return new Match(log, classDecl, source.split("\\.")[0].equals(name));
}
#+END_SRC

because I didn't have the metamodels loaded.  Still, it's nearly instantaneous
to match the 100k instances, and takes less than 1sec to match 1M of them!

I think it would be possible to write an Epsilon driver that turns a match AST
into a Java lambda... For genericity, it might be best.  Otherwise, it would
also be possible to walk the AST and produce one ourselves.  The first case
would be a great hack though... not unsimilar to what the Epsilon people did
for SQL queries (created from EOL queries).

* [2019-09-10 mar.]
** Partial evaluation of ECL matching rules                             :ecl:
Trying to do a prototype: load an ECL file, extract rules, recompile the rule,
and execute the compiled rule.

Wading through the maze of EOL interpreter... and trying to return suspended
lambdas.

For the compare block:

: return s.name.split("\\.")[0] = t.name;

Ideally I want to turn it into this:

#+BEGIN_SRC java
left, right -> {
  ((String) left.eGet("name"))
  .split("\\.")
  [0].equals(right.eGet("name"))
#+END_SRC

Now the problem is that I'm interpreting the AST, which is something like:

#+BEGIN_SRC elisp
(equals (aref 0 (call (get s "name") "split" "\\."))
        (get t "name"))
#+END_SRC

When interpreting ~equals~, the general form gets ~left~ and ~right~ arguments.
In a traditional eager interpreter, ~equals~ would get a string on the left, and
a string on the right, and would call equals on them.

In this lazy interpreter design, I get thunks:

#+BEGIN_SRC java
(get source prop) := if (source == left) left,right -> left.eGet(prop)
                     else                left,right -> right.eGet(prop)
(call receiver method ...args) := left,right -> invokeMethod(receiver, method, args)
(aref n array) := left,right -> Arrays.asList(array).get(n)
(equals l r) := left,right -> l.equals(r)
#+END_SRC

Which means that in a recursive interpreter we have thunks calling thunks,
instead of a just one statement block:

#+BEGIN_SRC java
left,right -> (
  (left,right -> Arrays.asList(
    (left,right -> invokeMethod((left,right -> left.eGet("name"))(left,right), "split", ["\\."]))
    (left,right)).get(0))(left,right))
  .equals((left,right -> right.eGet("name"))(left,right))
#+END_SRC

which is less efficient.

Also, the array index is an interesting case.  In the EOL interpreter, it is
interpreted by the class ItemSelectorExpression, which does this:
...
Execute eagerly the arguments, then test them and get the index from it.
Now, the way I've implemented things so far in the prototype is that
~expression~ is a lambda.  So of course it's not a collection.  I can return a
lambda that also implements the Collection interface, but then
CollectionUtil.asList will invoke the toArray method on the lambda, and I don't
know how large the returned array should be!

Had it returned a list, which was still an object, I could have returned a dummy
list which would have turned the ~get~ into a thunk.  But I can't deal with an
array.

So it seems the only way to do the transformation would be writing my own lazy
EOL interpreter.

Trying to hardcode the javaClass ECL rule as the thunkified version above to see
if the performance gain would be worth it in this form...  But it doesn't work!
For some reason, the feature is not populated.

Here are the hardcoded classes, saved for later maybe:
...
static interface Lambda {
  Object exec(EObject left, EObject right) throws EolRuntimeException;
}

public Match ruleJavaClassWithThunks(EObject left, EObject right) throws EolRuntimeException {
  Object source = ((Lambda) ((l,r) -> l.eGet(l.eClass().getEStructuralFeature("source")))).exec(left, right);
  Object name = ((Lambda) ((l,r) -> r.eGet(r.eClass().getEStructuralFeature("name")))).exec(left, right);

  Object split = ((Lambda) ((l,r) -> ReflectionUtil.invokeMethodSafe(source, "split", Arrays.asList("\\.")))).exec(left, right);
  Object aref = ((Lambda) ((l,r) -> Arrays.asList(split).get(0))).exec(left, right);
  Object res = ((Lambda) ((l,r) -> aref.equals(name))).exec(left, right);

  return new Match(left, right, (boolean) res);
}
#+END_SRC

* [2019-09-11 mer.]
** Benching ModelJoin                                    :modeljoin:emfviews:
Timings in seconds, sizes in bytes:

| Tool      | Trace size | Transform | Cold open | Hot open | Cold browse | Hot browse |  Disk size |
|           |        <r> |       <r> |       <r> |      <r> |         <r> |        <r> |        <r> |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| ModelJoin |       100k |       100 |         4 |        2 |          <1 |         <1 | 11,270,306 |
| ModelJoin |         1M |     >3600 |         - |        - |           - |          - |          - |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| EMF Views |       100k |         - |         3 |        2 |      43--92 |         <1 |        247 |
|           |         1M |         - |           |          |             |            |        248 |

What I did:

- Transform :: click on the run button for the launch task of the QVT-O
     transformation, wait for it to end.
- Cold open :: open the jointarget model in MoDisco, wait for the "loading"
     messages to disappear
- Hot open :: close the jointarget and open it again, wait for the "loading"
     messages to disappear
- Cold browse :: time to open the traces virtual features on a ClassDecl
- Hot browse ::  ibidem, but opening the same feature a second time

* [2019-09-12 jeu.]
** Benching ModelJoin                                    :modeljoin:emfviews:
Results for cold browsing virtual features are nice but not great.  Browsing all
virtual features except ~traces~ is instantaneous, but the overhead of ECL
interpretation is killing us in this case.

So I tried to implement a partial evaluator that turns the ECL compare block
into lambdas.  And it works!  Only 3sec to unfold the javaClass feature on the
1M trace model.

Now to document all that...

* [2019-09-13 ven.]
** Partial evaluation of ECL rules                             :ecl:emfviews:
Sometimes I feel like a wizard.

The ECL interpretation overhead was killing us, but with 150 lines of code I
wrote a partial evaluator that returns a lambda to be executed with left and
right elements to be compared, and it's at least 20 times faster.

I think it could be even faster if we compiled to Java bytecode directly, but
that would require a less straightforward transformation.

Right now, I should make it a bit more correct.  Yesterday I hardcoded the
~split~ and ~toLowerCase~ operations, but that doesn't work generally unless we
know the receiver is a string.  In general, we have to find the method via
reflection at runtime (in the lambda), which is slower.  If we had static type
information, then we could at least extract the method ahead of time, and just
do the reflective execution at runtime.

It looks like ECL can resolve types of expressions via a ~compile~ method.  Now
to figure how to make it work.

Found a way to pass the type of the parameters, by loading the metamodels,
finding the class, and setting that into the EOL compilation context.  But the
static analysis chokes on EEnum having no instance class.

So here we go again.  I can roll my own static analysis that will get me the
type of the expression, and if that type is unknown, then I can fallback on a
reflective call.  But that may be too much work for now.  Let's roll with the
reflective call and see if that is too slow.


| Tool      | Trace size | Transform | Cold open | Hot open | Cold browse | Hot browse |  Disk size |
|           |        <r> |       <r> |       <r> |      <r> |         <r> |        <r> |        <r> |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| ModelJoin |        10k |        11 |         1 |        1 |          <1 |         <1 |  1,116,306 |
|           |       100k |       100 |         4 |        2 |          <1 |         <1 | 11,270,306 |
|           |         1M |     >3600 |         - |        - |           - |          - |          - |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| EMF Views |         1k |         - |        93 |       88 |           2 |         <1 |        245 |
|           |        10k |         - |     >1600 |        - |           - |          - |        246 |
|           |       100k |         - |     >1000 |        - |           - |          - |        247 |
|           |         1M |         - |         - |        - |           - |          - |        248 |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| EMF Views |         10 |         - |         2 |        2 |          <1 |         <1 |        243 |
| lazy      |         1k |         - |         4 |        2 |           2 |         <1 |        245 |
|           |        10k |         - |         2 |        2 |           2 |         <1 |        246 |
|           |       100k |         - |         3 |        2 |      34--92 |         <1 |        247 |
|           |         1M |         - |        19 |       14 |       >1000 |         <1 |        248 |
|-----------+------------+-----------+-----------+----------+-------------+------------+------------|
| EMF Views |       100k |         - |         6 |        3 |           3 |         <1 |        247 |
| compiled  |         1M |         - |        14 |       12 |           5 |         <1 |        248 |

It's noticeable at the 1M level, unfortunately.  Here we have only one virtual
feature in the model object, but if we had several... could be slow.  Still,
that's a 50 to 100x speedup!

In fact, now that ECL rules are compiled, we could probably compute them at view
loading time again reasonably.  But lazy might be the better default choice.
