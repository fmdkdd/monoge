...
* [2018-08-09 jeu.]
** Thinking about view maintenance                                 :efmviews:
...
Because if C continues to exist, it might reference B in its feature (might have
a feature of type B for instance).  What should we do with this feature?  Change
its type to C?  Remove it?

If C exists, does it get the feature inherited from B?

So, the simplest option is to C to be filtered as well.  That means:
